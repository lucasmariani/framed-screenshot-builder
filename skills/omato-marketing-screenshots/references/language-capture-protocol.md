...
- Set the actual simulator language/region before the first app launch, so system menus and pickers match as well as app strings. Prefer official tools when available; the verified CLI fallback for this dedicated simulator is `xcrun simctl spawn <UUID> defaults write NSGlobalDomain AppleLanguages -array en` and `xcrun simctl spawn <UUID> defaults write NSGlobalDomain AppleLocale -string en_US`. Substitute the audience's supported locale. Restart the simulator if system UI retains the previous locale; verify a system picker and the app's visible labels. Never apply these commands to the Mac's own defaults.
- Use official Xcode tools to build/install/run the saved checkout. Additionally launch with `-AppleLanguages (en)` and `-AppleLocale en_US`, replacing both for the audience. In Scan select Translation and Translate into English **before** loading the viewfinder image. Do not enable `UITEST_MODE` or Mock Vocabulary. Complete onboarding; Debug Subscription Override is sufficient for this disposable screenshot environment, without purchase or personal iCloud sign-in.
...
- **Scan — required order (Lucas, September 24, 2026):** Open Scan → **select Translation** → choose/verify the explanation language → dismiss first-run tips → open Settings → Debug Settings → Load Viewfinder Image → select the page. The app returns to the already-configured Translation viewfinder. Verify the target language and image, then capture. Do not load the image in Dictionary mode and switch afterward. If the brief explicitly calls for a same-language Dictionary demonstration, select Dictionary before loading instead and record that deliberate mode choice. The viewfinder uses the real app crop, so inspect the complete frame. Hide Manual Image Upload may be enabled for the clean Scan presentation. The image fixture is in memory and must be reloaded after app relaunch; restore the desired mode and target language before reloading it.
...
If the device uses 24-hour time and displays `09:41`, run `xcrun simctl spawn <UUID> defaults write NSGlobalDomain AppleICUForce24HourTime -bool false` and `xcrun simctl spawn <UUID> defaults write NSGlobalDomain AppleICUForce12HourTime -bool true`, then relaunch before loading fixtures. The verified English set displays `9:41`. Preserve the appropriate audience's localized UI while retaining this marketing status-bar convention.
