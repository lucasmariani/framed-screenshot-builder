...
**Subtitle readability default — Lucas, September 24, 2026:** Use **83 px** as the baseline for ad subtitle/support layers; increase it when a format needs larger type for phone-size readability. Keep Baskerville Regular (400). Adjust line breaks, text-box placement and surrounding layout to fit; do not quietly shrink subtitles to fit. Check at phone-feed size. This explicit newer instruction supersedes earlier smaller support sizes and the instruction to preserve their size.
Current Meta revision: `spanish-meta-v2`; the saved Reddit set remains separately preserved.


**Contrast and Stories correction — Lucas, September 24, 2026:** Never place dark copy over dark leaves, shadows, books or textured objects. Check the composed background beneath every text layer, not a palette swatch or the average canvas color. Reposition the background/copy or add a separate editable, softly feathered ivory contrast wash; preserve the photographic theme and genuine app captures. Run `tools/check_ad_contrast.cjs <project> <report>` with extracted fonts. Require at least 4.5:1 at all sampled glyph locations as an internal artwork standard, then inspect at 360–390 px feed width; this numerical check is not a complete accessibility audit.

Stories must be composed for 9:16, with prominent text and a large recognizable phone, not a small square layout floating in empty space. Use a full-width headline/support stack over a large phone; an ASC-style continuation below the canvas is allowed. Safe areas constrain marketing copy and explicitly identified essential UI proof, not the whole decorative device. Keep the frame top intact and the featured term/translation or checked-green answer visible within the guide. Do not loosen provider guides simply to fill the canvas; preview the actual placement before upload. The approved smaller support default is superseded where Stories need larger type: use 83 px as the baseline, with larger sizes when needed for readability. Current v3 Stories use 136 px headings, 90 px support and a 990 px-wide phone on a 1440 px-wide canvas. These are an example, not coordinates to copy blindly for every language. The rejected v1/v2 small-phone Stories layouts are not future templates.

Review: `meta-spanish-v3.html`; sources remain separate editable layers.
